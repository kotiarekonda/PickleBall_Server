...
var States = require('../States/States.model');
...
exports.photoupload = function(req, res){
  ...
  uploadingimages(req, res, req.files.ProfilePic, function(filename){
    User.findOne({_id: req.user._id}).lean().exec(function(err, userdata){
      if(err){ return handleError(res, err); }
      //var path = "http://localhost:9000/"+filename;
        User.update({_id: req.user._id}, {$set:{"PublicProfile1.ProfilePic": filename}}, function(err, playerupdates){
          if(err){ return handleError(res, err); }
          if(playerupdates.nModified){
            User.findOne({_id: req.user._id}).lean().exec(function(err, userdata){
              if(err){ return handleError(res, err); }
              res.status(200).json({"ProfilePic": userdata.PublicProfile1.ProfilePic});
            })
          }else{
            res.status(200).json("failed to upload profile pic");
          }
        })
    })
  });
...
function uploadingimages(req, res, file, callback){
  ...
  sampleFile.mv('./uploads/'+finalname, function(err) {
    //local point
    var path = "http://localhost:9000/"+finalname;
    //remote server point
    //var path = "http://62.151.179.196:9005/"+finalname;
     callback(path);
  })
}
...
exports.create = function (req, res, next) {
  ...
  Country.findOne({Country: req.body.Country},function(err, country){
    ...
    if(country !== undefined){
      req.body.Country = country.CountryCode;
      //States
      States.find({State: req.body.State}, function(err, state){
        if(err){ return handleError(res, err); }
        if(state.length > 0 ){
          req.body.State = state[0].StateCode;
...
function repeatedemailandusername(res, err) {
  if(err.errors.email !== undefined){
    return res.status(500).json(err.errors.email.message);
    ...
    return res.status(500).json(err.errors.hashedPassword.message);
  }
}
/**
 * Get a single user
...
exports.forgotpassword = function(req, res){
  User.find({
    ...
    if (userdtls.length > 0) {
      ...
      var Name = userdtls[0].FirstName+' '+ userdtls[0].LastName;
      ...
      uploadingimages(req, res, req.files.ProfilePic, function(filename){
        if(filename){
          User.findOne({_id: req.user._id}, function(err, playerprofile){
            if(err){ return handleError(res, err); }
            req.body.ProfilePic = filename;//"http://localhost:9000/"+filename;
            delete req.body.key;
        ...
        pics.forEach(function(element, index){
          ...
          uploadingimages(req, res, element, function(filename){
            ...
            if(count == pics.length){
              User.findOne({_id: req.user._id}, function(err, playerdetails){
                if(err){ return handleError(res, err); }
                if(playerdetails.Photos.length !== 0){
                  User.update({_id: req.user._id},{$pushAll: {Photos: arr}}, function(err, updatedplayer){
                    if(err){ return handleError(res, err); }
                    if(updatedplayer.nModified){
                      User.findOne({_id: req.user._id},{ salt: 0, hashedPassword: 0}).lean().exec(function(err, playerinfo){
                        if(err){ return handleError(res, err); }
                        playerinfo.PublicProfile = {};
                        if(playerinfo.PublicProfile1 !== undefined){
                  ...
                  playerdetails.Photos = arr;
                  playerdetails.save(function(err, profiledata){
                    if(err){ return handleError(res, err); }
                    User.findOne({_id: req.user._id},{ salt: 0, hashedPassword: 0}).lean().exec(function(err, playerinfo){
                      if(err){ return handleError(res, err); }
                      playerinfo.PublicProfile = {};
        ...
        uploadingimages(req, res, req.files['Photos[]'], function(filename){
          User.findOne({_id: req.user._id}, function(err, player){
            ...
            if(player.Photos.length !== 0){
              //push image urls to Photos arr
              User.update({_id: req.user._id},{$pushAll:{Photos: [filename]}}, function(err, playerdata){
                if(err){ return handleError(res, err); }
                if(playerdata.nModified){
                  User.findOne({_id: req.user._id},{ salt: 0, hashedPassword: 0}).lean().exec(function(err, playerinfo){
                    if(err){ return handleError(res, err); }
                    playerinfo.PublicProfile = {};
                    if(playerinfo.PublicProfile1 !== undefined){
                  ...
                  res.status(200).json('failed to image upload');
                }
              })
            }else{
              player.Photos = [path];
...
exports.playerlookupsearch = function(req, res){
  ...
  var countqry;
  if(req.body.Letter){
    searchqry = User.find({LastName: {$regex: '^' + req.body.Letter, $options: 'i'}, role:'Player'},{FirstName: 1, LastName: 1, PublicProfile1: 1, City: 1, State: 1, Country: 1,  normalized: 1}).lean().limit(100).skip(req.body.Skip).sort('normalized');
    countqry =  User.find({LastName: {$regex: '^' + req.body.Letter, $options: 'i'}, role:'Player'},{FirstName: 1, LastName: 1, PublicProfile1: 1, City: 1, State: 1, Country: 1,  normalized: 1}).lean().sort('normalized');
  }else if(req.body.Gender){
    searchqry = User.find({Gender: req.body.Gender, role:'Player'},{FirstName: 1, LastName: 1, PublicProfile1: 1, normalized: 1, City: 1, State: 1, Country: 1} ).lean().sort('normalized').limit(100).skip(req.body.Skip);
    countqry = User.find({Gender: req.body.Gender, role:'Player'},{FirstName: 1, LastName: 1, PublicProfile1: 1, normalized: 1, City: 1, State: 1, Country: 1} ).lean().sort('normalized');
    ...
    searchqry = User.find({$or:[
      ...
      ], role: 'Player'}, {FirstName: 1, LastName: 1, City: 1, State: 1, Country: 1, PublicProfile1: 1, normalized: 1}).lean().sort('normalized').limit(100).skip(req.body.Skip);
    countqry = User.find({$or:[
      {FirstName: {$regex: '^' + req.body.Name, $options: 'i'}},
      {LastName: {$regex: '^' + req.body.Name, $options: 'i'}},
      {City: {$regex: '^' + req.body.Name, $options: 'i'}},
      {State: {$regex: '^' + req.body.Name, $options: 'i'}},
      {Country: {$regex: '^' + req.body.Name, $options: 'i'}},
      {HomeClub: {$regex: '^' + req.body.Name, $options: 'i'}}
      ], role: 'Player'}, {FirstName: 1, LastName: 1, City: 1, State: 1, Country: 1, PublicProfile1: 1, normalized: 1}).lean().sort('normalized');
  }else if(req.body.SkillRatingBy){
    searchqry = User.find({DoublesSkillLevel: req.body.SkillRatingBy, role: 'Player'},{FirstName: 1, LastName: 1, PublicProfile1: 1,  normalized: 1, City: 1, State: 1, Country: 1}).lean().sort('normalized').limit(100).skip(req.body.Skip);
    countqry  = User.find({DoublesSkillLevel: req.body.SkillRatingBy, role: 'Player'},{FirstName: 1, LastName: 1, PublicProfile1: 1,  normalized: 1, City: 1, State: 1, Country: 1}).lean().sort('normalized');
  }
  searchqry.exec(function(err, userdetails){
    if(err){ return handleError(res, err);}
    if(userdetails.length > 0){
      countqry.exec(function(err, count){
        if(err){ return handleError(res, err); }
